...
public interface CertificadoRepository extends JpaRepository<Certificado, Integer> {
    ...
    @Query(value = "SELECT ce.id_certificado, ce.trilha, ce.dificuldade, ce.data_emitida, u.nome, u.sobrenome " +
            "FROM ( " +
            "    SELECT ce.id_certificado, ce.trilha, ce.dificuldade, ce.data_emitida, u.nome, u.sobrenome " +
            "    FROM VS_13_EQUIPE_9.CERTIFICADO ce " +
            "    INNER JOIN VS_13_EQUIPE_9.USUARIO u ON ce.ID_USUARIO = u.ID_USUARIO " +
            "    WHERE ce.ID_USUARIO = ?1 " +
            "    ORDER BY ce.data_emitida DESC " +
            ") c " +
            "WHERE ROWNUM <= 1", nativeQuery = true)
    Certificado listarUltimo(Integer idUsuario);

    @Query(value = "SELECT c.trilha, c.dificuldade, c.data_emitida, u.nome, u.sobrenome " +
                "FROM CERTIFICADO c " +
