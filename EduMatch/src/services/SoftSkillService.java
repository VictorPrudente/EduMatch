...
public class SoftSkillService {
    ...
    private List<SoftSkill> perguntas = new ArrayList<>();
    ...
    public boolean salvar(SoftSkill novaQuestao) {
        for (SoftSkill questaoExistente : perguntas) {
    ...
    public void listarTodos(){
        for (SoftSkill questao : perguntas){
    ...
    public void atualizar(int id, String textoAtualizado, String opcaoCorreta){
        Optional<SoftSkill> questao = perguntas.stream().filter(questao1 -> questao1.getId() == id).findFirst();
    ...
    public void deletar(int id){
        for (SoftSkill questaoADeletar : perguntas){
            if (questaoADeletar.getId() == id){
                perguntas.remove(questaoADeletar);
            }
        }
    }
}
